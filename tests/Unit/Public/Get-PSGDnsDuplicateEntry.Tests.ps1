...
Describe 'Get-PSGDnsDuplicateEntry' {
    ...
    Context 'When called with ComputerName' {
        BeforeAll {
            Mock -CommandName Get-DnsServerZone -MockWith { $script:mockZones } -ModuleName $script:moduleName
            Mock -CommandName Get-DnsServerResourceRecord -MockWith { $script:mockRecordsWithDuplicates } -ModuleName $script:moduleName
        }
        ...
        It 'Should pass ComputerName to Get-DnsServerResourceRecord' {
            Get-PSGDnsDuplicateEntry -ComputerName 'dc01.contoso.com' -ZoneName 'contoso.com'
            Should -Invoke -CommandName Get-DnsServerResourceRecord -ParameterFilter { $ComputerName -eq 'dc01.contoso.com' } -ModuleName $script:moduleName
        }
    }
